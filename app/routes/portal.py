...
async def prepare_and_tariffs(request: Request, mac: str, router_id: str = "astana_01", cid: str = ""):
    ...
    if is_android:
        # Android: fire-and-forget PAY_WINDOW + мгновенный мета-редирект через bridge.
    ...
    # Так iOS не зависает на loading.html: CNA не обнаруживает интернет в фоне.
...
async def open_payment(
    ...
    """iOS: CNA идёт сюда (HTTP). Создаём PAY_WINDOW, редиректим на HTTPS.
    iOS не может обработать HTTPS в CNA → открывает Safari автоматически и CNA закрывается.
    PAY_WINDOW создаётся только здесь — CNA не видит интернет на этапе тарифов."""
    cid = (cid or "-")[:24]
    ...
    logger.info("[open_payment] PAY_WINDOW ready → Safari cid=%s mac=%s*** router=%s", cid, mac[:8], router_id)
